...
process.title = 'http-proxy-server ' + HTTP_PORT +
...
*/
  ...
  if (PROXY_URL)
    var options = {host: PROXY_HOST, port: PROXY_PORT, path: cliReq.url,
                   method: cliReq.method, headers: reqHeaders, agent: agent};
  else
...
server.on('connect', function onCliConn(cliReq, cliSoc, cliHead) {
  ...
  if (PROXY_HOST) {
    ...
    svrReq.on('connect', function onSvrConn(svrRes, svrSoc2, svrHead) {
      ...
      if (IS_TRACE) {
        log.trace('%s', '%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%');
        log.trace('HTTP/1.0 200 Connection established\n');
      }
...
var whiteAddressList = {
  ...
  '192.168.250.11' : true,
  '192.168.250.88' : true,
  '192.168.250.128': true,
  '192.168.250.129': true,
  '192.168.251.1'  : true,
  '192.168.251.10' : true,
  '192.168.251.11' : true,
  '192.168.251.88' : true,
  '192.168.251.128': true,
  '192.168.251.129': true,
  '192.168.100.3'  : true,
  '192.168.101.21' : true,
  '192.168.101.140': true,
  '172.17.68.88': true};
...
server.on('connection', function onConn(cliSoc) {
  ...
  cliSoc.$httpAgent = new http.Agent({keepAlive: true});
  // cliSoc.$httpAgent.maxSockets = 1;
  cliSoc.$serverRequested = false;

  cliSoc.$httpAgent.on('error', function onAgentErr(err) {
